...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public long createCharacter(Character character){
        ...
        for (int majorSkill : majorSkills) {
            majorSkillsValues.put(KEY_CHARACTER_ID, Long.toString(character.getId()));
            majorSkillsValues.put(KEY_SKILL_ID, majorSkill);
	...
	public List<Skill> getSkillsForCharacter(String character){
		...
		List<Skill> skills = new ArrayList<>(27);
		for(int i = 0; i < 27; i++){
			...
			if(maxLevel < getCharacterLevel(character)){
				actSkillLevel = getActSkillLevel(charId, i);
				...
				actSkillLevel = getActSkillLevel(charId, i);
				prevSkillLevel = getPrevSkillLevel(charId, i);
				if(prevSkillLevel == 0){
					prevSkillLevel = actSkillLevel;
				}
			...
			curIsMajor.close();
			skills.add(new Skill(i, actSkillLevel, prevSkillLevel, overLevel, governingAttribute, isMajor));
		}
		db.close();
		return skills;
	}
	...
	public int getMultiplierForAttribute(String character, int attrId) {
        ...
        for (int governedSkill : governedSkills) {
            sumForAttr += getActSkillLevel(charId, governedSkill) - getPrevSkillLevel(charId, governedSkill);
        }
		return sumForAttr;
	}
